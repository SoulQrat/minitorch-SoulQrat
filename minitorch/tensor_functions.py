...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        return grad_output.f.add_zip(
                    grad_output.f.mul_zip(grad_output, s), 
                    grad_output.f.neg_map(
                        grad_output.f.mul_zip(grad_output.f.mul_zip(grad_output, s), s))
                )
...
class Exp(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (t1,) = ctx.saved_values
        return t1.f.exp_map(t1).f.mul_zip(grad_output, t1.f.exp_map(t1))
...
class LT(Function):
    ...
    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        return zeros(ctx.saved_values[0].shape), zeros(ctx.saved_values[1].shape)
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        order = [int(el) for el in order._tensor._storage]
